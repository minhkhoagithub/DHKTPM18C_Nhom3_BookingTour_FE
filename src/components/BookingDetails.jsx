...
const BookingDetail = () => {
  ...
  const handleCancel = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:8080/api/bookings/invoice/${booking.invoiceId}/cancel`,
        {
          method: "PUT",
        }
      );
  ...
  if (loading)
    return (
      <div className="min-h-screen pt-24 text-center">Đang tải chi tiết...</div>
    );
  if (error)
    return (
  ...
  const handlePayment = () => {
    if (!booking || !booking.invoiceId) {
      alert("Lỗi: Đơn hàng chưa có hóa đơn để thanh toán.");
      return;
    }

    // Chuyển hướng sang trang Payment kèm dữ liệu
    navigate("/payment", {
      state: {
        invoiceId: booking.invoiceId,
        totalAmount: booking.totalPrice,
        promotionRef: booking.promotionRef,
          ...
          <div className="p-6 grid gap-8">
            ...
            <section>
              <h3 className="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2">
                <MapPin className="text-blue-600" size={20} /> Thông tin Chuyến
                đi
              </h3>
              <div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
                ...
                <div className="grid md:grid-cols-2 gap-4 text-sm text-gray-700">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span>
                      <b>Kết thúc:</b> {formatDate(booking.departure?.endDate)}
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    <MapPin size={16} className="text-gray-400" />
                    <span>
                      <b>Địa điểm:</b> {booking.departure?.tour?.location}
                    </span>
                  </div>
                </div>
              </div>
            ...
            <section>
              <h3 className="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2">
                <Users className="text-blue-600" size={20} /> Danh sách Hành
                khách ({booking.passengers?.length || 0})
              </h3>
            ...
            <div className="flex justify-end gap-4 pt-4 border-t border-gray-200">
              ...
              {booking.status === "HELD" && (
                <button
                  ...
                  className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-semibold flex items-center gap-2 shadow-lg shadow-blue-200 transition-transform hover:-translate-y-0.5"
                >
