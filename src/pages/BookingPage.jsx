...
export default function BookingPage() {
  ...
  const handleSubmitBooking = async (e) => {
    ...
    try {
      ...
      if (!invoiceId) {
        console.error(
          "Lỗi: BE không trả về invoiceId. Kiểm tra BookingServiceImpl."
        );
    ...
    <>
      ...
      <div className="max-w-4xl mx-auto py-12 px-4">
        ...
        <div className="bg-gray-50 p-4 rounded-lg mb-6">
          <p>
            <strong>Tour ID:</strong> {tourId}
          </p>
          <p>
            <strong>Departure ID:</strong> {departureId}
          </p>
          <p>
        ...
        <form onSubmit={handleSubmitBooking}>
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            <h3 className="text-xl font-bold mb-4">
              Thông tin Hành khách (Tổng: {guests})
            </h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="md:col-span-2 flex items-center">
                    <input
                      ...
                      onChange={(e) =>
                        handleRoomSingleChange(index, e.target.checked)
                      }
                    />
                    <label
                      htmlFor={`roomSingle-${index}`}
                      className="ml-2 block text-sm text-gray-900"
                    >
                      Yêu cầu phòng đơn (phụ thu)
                    </label>
                  </div>
                </div>
              </div>
          ...
          <button
            type="submit"
            className="w-full mt-6 bg-red-500 text-white py-3 px-4 rounded-md hover:bg-red-600 font-semibold text-lg"
          >
            Xác nhận & Đặt Tour
          </button>
        </form>
