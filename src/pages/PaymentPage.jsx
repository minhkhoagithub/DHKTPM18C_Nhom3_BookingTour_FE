...
const qrUrl = `https://img.vietqr.io/image/${BANK_ID}-${ACCOUNT_NUMBER}-compact2.png?amount=${totalAmount}&addInfo=PAY-${invoiceId}&accountName=${encodeURIComponent(ACCOUNT_NAME)}`;
  ...
  const handleConfirmPayment = async () => {
    ...
    try {
      alert(
        "Đang xác nhận thanh toán... vui lòng chờ. Thông tin thanh toán sẽ được gửi qua email."
      );
      console.log();
      navigate("/tours");
    } catch (err) {
  ...
  const handleCancel = async () => {
    if (
      !window.confirm(
        "Bạn có chắc muốn hủy booking này? (Chỉ có thể hủy khi chưa thanh toán)"
      )
    ) {
      return;
    }
        ...
        <div className="bg-white p-6 rounded-lg shadow-lg">
          <div className="w-48 h-48 bg-gray-200 mx-auto flex items-center justify-center">
            <img
              src={qrUrl}
              alt="QR VietQR"
              className="mx-auto w-64 shadow-lg rounded-md"
            />
          </div>
